...
export default class SurveyForm extends React.Component {
  ...
  state_to_csv() {
    ...
    keys.forEach(k => {
      if (
        this.state.submit_vals[k] instanceof Object &&
    ...
    let component_array = this.state.input_type_array.map((prop, key) => {
      if (prop == "ScaleSlideInputType") {
        let numOfOptions = this.state.valOptions[this.state.value_labels[key]]
          ? this.state.valOptions[this.state.value_labels[key]].length
          : 1;
        return (
          <ScaleSlideInputType
            key={key}
            ...
            scale_labels={
              this.state.valOptions[this.state.value_labels[key]] || []
            }
            ...
            onStart={() => {
              this._form.disable_swipe();
            }}
            onComplete={() => {
              this._form.enable_swipe();
            }}
          />
        );
